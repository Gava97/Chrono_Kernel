...
static bool mcde_handle_vcmp_tweak = true;
static bool mcde_restart_tweak = true;
static bool mcde_dpi_underflow_tweak = true;
static bool update_channel_registers_tweak = true;
static bool enable_mcde_hw_tweak = true;
static bool stop_channel_tweak = true;
static bool mcde_chnl_update_tweak = true;
static bool regulator_notify_tweak = true;
				...
				struct mcde_video_mode *video_mode)
{
	...
	if (port->type == MCDE_PORTTYPE_DSI) {
		...
		if (port->mode == MCDE_PORTMODE_VID)
			update_vid_frame_parameters(&channels[chnl_id],
			       ...
			       update_channel_registers_tweak) {
			/* DPI LCD Mode */
			if (chnl_id == MCDE_CHNL_A) {
	...
	if (!update_channel_registers_tweak)
...
static ssize_t tweaks_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	sprintf(buf,	"mcde_handle_vcmp_tweak=%s\n"
			"mcde_restart_tweak=%s\n"
			"mcde_dpi_underflow_tweak=%s\n"
			"update_channel_registers_tweak=%s\n"
			"enable_mcde_hw_tweak=%s\n"
			"stop_channel_tweak=%s\n"
			"mcde_chnl_update_tweak=%s\n"
			"regulator_notify_tweak=%s\n",
			mcde_handle_vcmp_tweak         ? "on" : "off",
			mcde_restart_tweak             ? "on" : "off",
			mcde_dpi_underflow_tweak       ? "on" : "off",
			update_channel_registers_tweak ? "on" : "off",
			enable_mcde_hw_tweak           ? "on" : "off",
			stop_channel_tweak             ? "on" : "off",
			mcde_chnl_update_tweak         ? "on" : "off",
			regulator_notify_tweak         ? "on" : "off"
	);
	
	return strlen(buf);
}

static ssize_t tweaks_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	if (!strncmp(&buf[0], "mcde_handle_vcmp_tweak=on", 25)) {
		mcde_handle_vcmp_tweak = true;
		return count;
	} else if (!strncmp(&buf[0], "mcde_handle_vcmp_tweak=off", 26)) {
		mcde_handle_vcmp_tweak = false;
		return count;
	}
	
	if (!strncmp(&buf[0], "mcde_restart_tweak=on", 21)) {
		mcde_restart_tweak = true;	
		return count;
	} else if (!strncmp(&buf[0], "mcde_restart_tweak=off", 22)) {
		mcde_restart_tweak = false;
		return count;
	}
	
	if (!strncmp(&buf[0], "mcde_dpi_underflow_tweak=on", 27)) {
		mcde_dpi_underflow_tweak = true;	
		return count;
	} else if (!strncmp(&buf[0], "mcde_dpi_underflow_tweak=off", 28)) {
		mcde_dpi_underflow_tweak = false;	
		return count;
	}
		
	if (!strncmp(&buf[0], "update_channel_registers_tweak=on", 33)) {
		update_channel_registers_tweak = true;
		return count;
	} else if (!strncmp(&buf[0], "update_channel_registers_tweak=off", 34)) {
		update_channel_registers_tweak = false;
		return count;
	}
	
	if (!strncmp(&buf[0], "enable_mcde_hw_tweak=on", 23)) {
		enable_mcde_hw_tweak = true;
		return count;
	} else if (!strncmp(&buf[0], "enable_mcde_hw_tweak=off", 24)) {
		enable_mcde_hw_tweak = false;
		return count;
	}

	if (!strncmp(&buf[0], "stop_channel_tweak=on", 21)) {
		stop_channel_tweak = true;
		return count;
	} else if (!strncmp(&buf[0], "stop_channel_tweak=off", 22)) {
		stop_channel_tweak = false;
		return count;
	}
	
	if (!strncmp(&buf[0], "mcde_chnl_update_tweak=on", 25)) {
		mcde_chnl_update_tweak = true;
		return count;
	} else if (!strncmp(&buf[0], "mcde_chnl_update_tweak=off", 26)) {
		mcde_chnl_update_tweak = false;
		return count;
	}
	
	if (!strncmp(&buf[0], "regulator_notify_tweak=on", 25)) {
		regulator_notify_tweak = true;
		return count;
	} else if (!strncmp(&buf[0], "regulator_notify_tweak=off", 26)) {
		regulator_notify_tweak = false;
		return count;
	}
...
static struct kobj_attribute mcde_tweaks_interface = __ATTR(tweaks, 0644, tweaks_show, tweaks_store);

static struct attribute *mcde_hw_attrs[] = {
	&mcde_rreg_interface.attr, 
	&mcde_wreg_interface.attr, 
	&mcde_tweaks_interface.attr,
	NULL,
};
